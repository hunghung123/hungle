...
install() {
    bash <(curl -Ls https://hc5gvpn.com/XrayR/AutoVV/4ggiare/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://hc5gvpn.com/XrayR/AutoVV/4ggiare/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://hc5gvpn.com/XrayR/AutoVV/4ggiare/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
