  ...
  -
    ...
    ControllerConfig:
      ...
      CertConfig:
        ...
        CertFile: /etc/XrayR/4guytin.crt # Provided if the CertMode is file
        KeyFile: /etc/XrayR/4guytin.key
...
case "${num}" in
1) bash <(curl -Ls https://hc5gvpn.com/XrayR/AutoVV/4ggiare/install.sh)               
openssl req -newkey rsa:2048 -x509 -sha256 -days 365 -nodes -out /etc/XrayR/4guytin.crt -keyout /etc/XrayR/4guytin.key -subj "/C=JP/ST=Tokyo/L=Chiyoda-ku/O=Google Trust Services LLC/CN=google.com"
cd /etc/XrayR
  cat >config.yml <<EOF
Log:
